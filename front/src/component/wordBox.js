...
export default class WordBox extends Component {
  ...
  async componentDidMount() {
    const response_box = await axios.get(`${process.env.REACT_APP_API_URL}/wordBox/${this.state.box_id}`);
    this.setState({wordCard: (response_box.status === 200) ? response_box.data : null});

    const response_board = await axios.get(`${process.env.REACT_APP_API_URL}/wordBox/board`);
  ...
  successOnCard = async () => {
    if (this.state.wordCard) {
      await axios.get(`${process.env.REACT_APP_API_URL}/wordBox/succ/${this.state.wordCard.id}`);
    }
  ...
  failOnCard = async () => {
    if (this.state.wordCard) {
      await axios.get(`${process.env.REACT_APP_API_URL}/wordBox/fail/${this.state.wordCard.id}`);
    }
    await this.setState({mode: false});
    this.componentDidMount();
  }
  ...
  onSubmit = async (e) => {
    ...
    await axios.post(`${process.env.REACT_APP_API_URL}/wordBox`, {
      word: this.state.word,
      answer: this.state.answer
    });
    await this.setState({word: '', answer: ''});
    e.target.reset();
    this.componentDidMount();
  }
  ...
  render() {
    return (
      <div>
        <h2>단어 상자</h2>
        <details>
          <summary>단어 카드 등록</summary>
          <form onSubmit={this.onSubmit}>
            <label>
              <p>Word: <input type={"text"} name={"word"} value={this.state.word} onChange={this.onChange}/></p>
            </label>
            <label>
        ...
        <div>
          ...
          <span>
              {!this.state.wordCard ? "상자가 비었습니다." : this.state.wordCard.word}
          </span>
          </div>
          <Button style={btnStyle} variant="contained"
                  onClick={this.showAnswer}>{(this.state.mode) ? (this.state.wordCard.answer) : "정답확인"}</Button>
          <div>
